...
class FattureInCloudOAuthController extends Controller
{
    ...
    private function createOrUpdateFicAccount(
        ...
        // Normalize to array of [id, name] for consistent handling
        $companiesList = is_array($companies) ? $companies : [$companies];
        $normalized = [];
        foreach ($companiesList as $c) {
            $id = is_object($c) ? $c->getId() : $c['id'] ?? null;
            $name = is_object($c) ? $c->getName() : $c['name'] ?? null;
            if ($id !== null) {
                $normalized[] = ['id' => (int) $id, 'name' => $name];
            }
        }
        if (empty($normalized)) {
        ...
        $selected = FicOAuthCompanySelector::select($normalized, $tenantId);
        ...
        $existingOther = FicAccount::where('company_id', $companyId)
            ->where('tenant_id', '!=', $tenantId)
            ->first();

        if ($existingOther !== null) {
            $existingTeam = \App\Models\Team::find($existingOther->tenant_id);
            Log::warning('FIC OAuth: Company ID already connected to another team', [
                'company_id' => $companyId,
                'company_name' => $companyName,
                'current_team_id' => $tenantId,
                'existing_team_id' => $existingOther->tenant_id,
